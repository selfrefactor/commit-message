import { getter, setter } from 'client-helpers-fn'
import { replace, shuffle } from 'rambdax'
import { maskSentence, takeArguments, wordsX } from 'string-fn'
...
export function chitankaBee(){
  ...
  const els = Array.from(document.querySelector(SELECTOR).querySelectorAll('p'))
  ...
  els.forEach((element, i) => {
    ...
    const newHtml = replace(
      toReplace, replacer, element.innerHTML
    )
