...
function getFolder(){
  return process.env.BENCHMARK_FOLDER ? process.env.BENCHMARK_FOLDER : folderFallback
}
...
async function createBenchmark({label: topLabel, suites}){
  ok(suites, topLabel)([{label: String, fn: Function}], String)

  const benches = suites.map(
