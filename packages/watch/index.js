  ...
  try {
    ...
    for (const singleCommand of commands) {
      ...
      if (R.type(singleCommand) === 'String') {
        const command = R.replace(/filepath/g, filePath, singleCommand)

        holder = await execFileCommand(
          command,
          options.cwd
        )
...
async function main ({ filePath, commands, options }) {
  try {
    ...
    if (R.type(commands) === 'String') {
      const command = R.replace(/filepath/g, filePath, commands)

      willReturn = await execFileCommand(
        command,
        options.cwd
      )
