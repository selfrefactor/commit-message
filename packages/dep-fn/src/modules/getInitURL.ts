import {compose, length, match, path, replace} from 'rambdax'
...
export const getInitURL = async(dependency: string): Promise<string> => {
  ...
  try {
    ...
    const startCondition: boolean = urlGithub.startsWith(
      'https://github.com'
    )
